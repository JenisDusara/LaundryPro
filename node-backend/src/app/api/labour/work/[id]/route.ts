...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing: any = await withRetry(() => prisma.labourWork.findFirst({
    where: { id: params.id, deleted_at: null, ...scope },
    include: { labour: true },
  } as any));
  if (!existing) return NextResponse.json({ detail: "Not found" }, { status: 404 });
  const reason = new URL(req.url).searchParams.get("reason") || "labour_work_delete";
  await withRetry(() => prisma.labourWork.update({
    where: { id: params.id },
  ...
  await logDataAction(req, user, {
    action: "labour_work.soft_deleted",
    shop_id: existing.labour.shop_id,
    entity_type: "labour_work",
    entity_id: existing.id,
    entity_label: `${existing.labour.name} ${existing.work_date}`,
    metadata: { reason, press_count: existing.press_count, rate_per_piece: Number(existing.rate_per_piece) },
  });
