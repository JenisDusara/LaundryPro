            ...
            <div key={svc.id} style={{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 2px 8px rgba(0,0,0,0.06)",borderTop:`3px solid ${color.icon}`}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  {svc.children.length>0?<div style={{fontSize:12,color:"#94a3b8"}}>{svc.children.length} sub-items</div>:svc.price!==null?<div style={{display:"flex",alignItems:"center",gap:2,fontWeight:700,fontSize:14,color:color.icon}}><IndianRupee size={12}/>{svc.price}</div>:null}
                </div>
                <div style={{display:"flex",gap:4}}>
                  {!svc.parent_id&&<button style={{width:28,height:28,border:"none",borderRadius:6,background:"#f8fafc",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>{setForm({name:"",price:"",parent_id:svc.id});setEditId(null);setShowForm(true);}}><Plus size={15} color="#3b82f6"/></button>}
